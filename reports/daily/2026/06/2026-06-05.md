...
- Must Read count: 3 (Adaptive Parallel Reasoning: The Next Paradigm in Efficient Inference Scaling；Masked Skill Token Training for Hierarchical Off-Dynamics Transfer；Gradient-based Planning for World Models at Longer Horizons)
- Skim count: 8 (Whole-Body Conditioned Egocentric Video Prediction；Scaling Up Reinforcement Learning for Traffic Smoothing: A 100-AV Highway Deployment；RL without TD learning；AgentRefine: Enhancing Agent Generalization through Refinement Tuning；Introducing new capabilities to GPT-Rosalind)
- Watch count: 12 (ShieldAgent: Shielding Agents via Verifiable Safety Policy Reasoning；Q-RAG: Long Context Multi‑Step Retrieval via Value‑Based Embedder Training；Identifying Interactions at Scale for LLMs；FrugalRAG: Less is More in RL Finetuning for Multi-hop Question Answering；Monte Carlo Planning with Large Language Model for Text-Based Game Agents)
- Keywords: environment、berkeley.edu、evaluation、framework、attention、diffusion、inference、planning
...
#### Skim
##### 1. [EpiCache: Episodic KV Cache Management for Long-Term Conversation on Resource-Constrained Environments](https://machinelearning.apple.com/research/epicache)
- 阅读层级：SKIM
- 来源：Apple Machine Learning Research
- 来源类型：一手来源
- source_role：institution_authority
- 证据来源：full text
- 原文链接：https://machinelearning.apple.com/research/epicache
- 发布时间：2026-05-19T00:00:00+00:00
- 这是什么？EpiCache: Episodic KV Cache Management for Long-Term Conversation on Resource-Constrained Environments 是一篇围绕 Context Compression / Long Context / Memory 的研究或技术文章；从正文摘要看，重点是：Modern large language models (LLMs) extend context lengths to millions of tokens, enabling coherent, personalized responses grounded in long conversational history. However, the Key-Value (KV) cache grows linearly with the extended dialogue history, causing the model's memory footprint to quickly exceed device limits. While recent KV cache compression methods attempt to reduce memory usage, most apply cache eviction after processing the entire context, incurring unbounded peak memory usage. Additionally, query-dep…
...
- 为什么对我重要？该来源具备 full text grounding，适合用作当天判断 Context Compression / Long Context / Memory 方向变化的实质材料；personal=0.69, relevance=0.64。
- 是否建议深读？建议略读正文，先抓住问题定义和方法框架。
- 建议行动：skim
- 评分：global_score 0.29；personal_score 0.69；credibility 1.00；conference 0.00；institution 0.00；multi_source 0.00；community_signal 0.08；actionability 0.40；research_relevance 0.64；hype_risk 0.00
...
- 命中方向：上下文压缩 / 长上下文 / 记忆
- 相关标签：NLP
- 命中关键词：KV cache、apple.com、dialogue、language model
...
- [Twilight: Adaptive Attention Sparsity with Hierarchical Top-$p$ Pruning](https://openreview.net/forum?id=Ve693NkzcU)（WATCH，Context Compression / Long Context / Memory，证据 abstract only，personal 0.74，global 0.27）
...
- [Monte Carlo Planning with Large Language Model for Text-Based Game Agents](https://openreview.net/forum?id=r1KcapkzCt)（WATCH，Agent / Reasoning / Inference-time Scaling / Planning，证据 abstract only，personal 0.90，global 0.28）
...
- [GitHub's plan for Agents — Kyle Daigle, GitHub](https://www.latent.space/p/github)（ARCHIVE，Agent / Reasoning / Inference-time Scaling / Planning，证据 full text，personal 0.59，global 0.31）
...
- [The next chapter in flood resilience: Open sourcing Google's hydrology framework](https://research.google/blog/the-next-chapter-in-flood-resilience-open-sourcing-googles-hydrology-framework/)（ARCHIVE，Other Highlights，证据 full text，personal 0.52，global 0.49）
- [A blueprint for democratic governance of frontier AI](https://openai.com/index/frontier-safety-blueprint)（ARCHIVE，Other Highlights，证据 full text，personal 0.51，global 0.48）
...
- [Extending Human Intelligence Through AI](https://www.microsoft.com/en-us/research/blog/extending-human-intelligence-through-ai/)（ARCHIVE，Other Highlights，证据 full text，personal 0.47，global 0.40）
- [Private analytics via zero-trust aggregation](https://research.google/blog/private-analytics-via-zero-trust-aggregation/)（ARCHIVE，Other Highlights，证据 full text，personal 0.47，global 0.40）
...
##### 4. [Towards Practical Defect-Focused Automated Code Review](https://openreview.net/forum?id=mEV0nvHcK3)
- 阅读层级：ARCHIVE
...
##### 5. [VSAS-Bench: Real-Time Evaluation of Visual Streaming Assistant Models](https://machinelearning.apple.com/research/vsas-bench-streaming-assistant)
- 阅读层级：ARCHIVE
- 来源：Apple Machine Learning Research
- 证据来源：full text
- benchmark 评估什么能力：评估摘要中描述的任务能力；具体指标需打开原文确认。
...
### Other Benchmarks
- 其余 11 个只进入附录标题列表：reports/appendix/2026-06-05-benchmarks.md
...
- 发布时间：2026-06-04T23:46:38+00:00
...
- [FingerTip 20K: A Benchmark for Proactive and Personalized Mobile LLM Agents](https://openreview.net/forum?id=n3iFV0gLMc)
  ...
  - 类型：paper
  - 为什么值得关注：institution_signal 0.96，authority_score 0.96
  - 与我的研究方向关系：Benchmark / Dataset / Evaluation，personal 0.89
  - 建议行动：watch
...
- 今日新论文继承了什么问题：Adaptive Parallel Reasoning: The Next Paradigm in Efficient Inference Scaling；Gradient-based Planning for World Models at Longer Horizons；Masked Skill Token Training for Hierarchical Off-Dynamics Transfer 继承了经典 agent 论文中的问题：如何把推理、行动、工具调用和环境反馈组织成可检查的轨迹。
...
- 相关今日条目：
  ...
  - [Gradient-based Planning for World Models at Longer Horizons](http://bair.berkeley.edu/blog/2026/04/20/grasp/)（Agent / Reasoning / Inference-time Scaling / Planning；连接词：agents、planning）
...
- [Gradient-based Planning for World Models at Longer Horizons](http://bair.berkeley.edu/blog/2026/04/20/grasp/)：预计阅读目的：提取 Agent 任务设定、工具使用方式、规划机制和评测指标。

## 12. Collection Notes
- Generated at: 2026-06-04T23:49:46.057015+00:00
- Source count: 25
- Raw item count: 447
- Dedup item count: 389
- Summary mode: single
- Provider: kimi
- Model: moonshot-v1-8k
...
- provider_disabled: none
- reason: none
- Benchmark appendix: reports/appendix/2026-06-05-benchmarks.md

- Report path: reports/daily/2026/06/2026-06-05.md
- Previous report link: reports/daily/2026/06/2026-06-04.md

## Source Health
- Hugging Face Daily Papers: error (0 items) - 429 Client Error: Too Many Requests for url: https://huggingface.co/api/daily_papers
...
- Papers with Code Trending (HF redirect): error (0 items) - 429 Client Error: Too Many Requests for url: https://huggingface.co/papers/trending
- GitHub AI Research Projects: time budget exhausted (25 items) - time budget exhausted after 25 items
- Stanford HAI: error (0 items) - 403 Client Error: Forbidden for url: https://hai.stanford.edu/news
- MIT CSAIL News: timeout (0 items) - timeout after 25s
- ACL: error (0 items) - 403 Client Error: Forbidden for url: https://www.aclweb.org/portal/news
